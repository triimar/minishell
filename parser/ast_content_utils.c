...
/*   Updated: 2023/06/20 19:51:09 by eunskim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token_scanner *scanner, t_ast *cmd_node)
{
	...
	word_cnt = 0;
	while (cmd_node->content->cmd != NULL \
	&& *(cmd_node->content->cmd + word_cnt) != NULL)
		word_cnt++;
	cmd_node->content->cmd = \
	extend_string_array(cmd_node->content->cmd, word_cnt);
	if (cmd_node->content->cmd == NULL)
	{
		data->malloc_failed = true;
		return (PARSER_FAILURE);
	}
	cmd_node->content->cmd[word_cnt] = \
	produce_dup_string(scanner->token_current->token.start, \
...
t_token_scanner *scanner, t_ast *cmd_node)
{
	...
	add_assignment_back(&(cmd_node->content->assignments), new_assignment);
	advance_token_list(scanner);
	return (PARSER_SUCCESS);
}
...
t_token_scanner *scanner, t_ast *cmd_node, t_redirect_type type)
{
	...
	if (type == REDIRECT_STDIN || type == REDIRECT_HERE_DOC)
		add_redirect_back(&cmd_node->content->stdin_redirect, new_redirect);
	if (type == REDIRECT_STDOUT || type == REDIRECT_STDOUT_APPEND)
		add_redirect_back(&cmd_node->content->stdout_redirect, new_redirect);
	advance_token_list(scanner);
	return (PARSER_SUCCESS);
}
